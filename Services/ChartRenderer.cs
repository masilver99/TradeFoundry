...
public static partial class ChartRenderer
{
    ...
    {
        var orderedBars = bars.OrderBy(x => x.EventUtc).ToArray();
        if (orderedBars.Length == 0) return Empty("No matching OHLCV bars yet. Import a bar file for this symbol and time window.");

        var entryX = NearestBar(orderedBars, trade.EntryUtc).EventUtc.ToString("O", CultureInfo.InvariantCulture);
        var shapes = new List<object> { MarkerShape(entryX, Gold) };
        var annotations = new List<object> { MarkerAnnotation(entryX, "entry", Gold) };
        if (trade.ExitUtc.HasValue)
        {
            var exitX = NearestBar(orderedBars, trade.ExitUtc.Value).EventUtc.ToString("O", CultureInfo.InvariantCulture);
            shapes.Add(MarkerShape(exitX, Ink));
            annotations.Add(MarkerAnnotation(exitX, "exit", Ink));
        }
        ...
        layout["xaxis"] = Axis();
        layout["xaxis"] = new Dictionary<string, object?>
        {
            ["type"] = "date",
            ["showgrid"] = true,
            ["gridcolor"] = Grid,
        ...
        SetAxis(layout, "yaxis", "price");
        return Plotly($"Candlestick chart for {trade.Symbol}", new object[]
        {
            new
            {
                type = "candlestick",
                x = orderedBars.Select(x => x.EventUtc.ToString("O", CultureInfo.InvariantCulture)).ToArray(),
                ...
                increasing = new { line = new { color = Green }, fillcolor = "rgba(63,185,80,.35)" },
                decreasing = new { line = new { color = Red }, fillcolor = "rgba(248,81,73,.35)" },
                whiskerwidth = .6,
    ...
    private static object MarkerAnnotation(string x, string text, string color) => new
    {
        x,
        y = 1,
        xref = "x",
        ...
        showarrow = false,
        yanchor = "bottom",
        font = new { color, family = "Segoe UI, system-ui, sans-serif", size = 10 }
    };

    private static object MarkerShape(string x, string color) => new
    {
        type = "line",
        x0 = x,
        x1 = x,
    ...
    private static string Plotly(string label, IEnumerable<object> traces, Dictionary<string, object?> layout)
    {
        var payload = new
        {
            data = traces.ToArray(),
    ...
    private static DateTimeOffset InZone(DateTimeOffset value, string? timeZoneId)
    {
        ...
        try { return TimeZoneInfo.ConvertTime(value, TimeZoneInfo.FindSystemTimeZoneById(id)); }
        catch (TimeZoneNotFoundException) { return value.ToLocalTime(); }
        catch (InvalidTimeZoneException) { return value.ToLocalTime(); }
    }

    private static Bar NearestBar(IReadOnlyList<Bar> bars, DateTimeOffset time) => bars
        .OrderBy(bar => Math.Abs((bar.EventUtc - time).TotalSeconds))
        .First();
